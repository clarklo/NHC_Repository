...
function registCheck(){
	if(document.registForm.loginname.value=="")
	{
		alert("帳號不能為空﹗");
		document.registForm.loginname.focus();
		return false;
	}
	if (document.registForm.loginname.value.length > 13 || document.registForm.loginname.value.length < 5)
    { 
        alert("帳號長度介於6-12個英文字母、數字組成﹗");
		document.registForm.loginname.focus();
		return false;
     }
	var txt1=window.document.registForm.loginname;
	var regs='^[a-z0-9A-Z]+$';
...
<form name="registForm" action="regist_do.jsp" method="post">
   ...
   <div class="FlowChart">
    ...
    <div class="Explanation">會員資料</div>
    </div>
    <div class="clear"></div> 
    <div class="write">
<table width="630" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <th><span>*</span>帳號：</th>
    <td>
	<input name="loginname" class="w_in10" class="input" id="loginname" size="16" maxlength="20"  onkeyup="^[a-zA-Z]\w{5,11}$" />
...
<p>5-20個字符(包括小寫字母、數字)</p>
  ...
  <tr>
    <th>出生年月日：</th>
    <td>
	<input name="age" class="w_in10" type="text" id="date" size="20" value="<%=formatter.format(currentTime_1)%>" onClick="J.calendar.get({dir:'right'});"/>
	</td>
  </tr>
  <tr>
