...
if(date1 == null || date1.equals("")){
	out.println("<script language=javascript> alert('上架日期不能為空,請輸入您的上架日期!');history.go(-1);</script>");
}
else if(date2 == null || date2.equals("")){
	out.println("<script language=javascript> alert('下架日期不能為空,請輸入文章的下架日期!'); history.go(-1);</script>");
}
////
...
if(!hasPermission){
	...
	date1 = ReplaceAll.toSql(date1);
	date2 = ReplaceAll.toSql(date2);
	...
	article.setTime1(date1);
	article.setTime2(date2);
	...
	if(processArticle.isStatus()){			
		out.println("<script language=javascript>location.href='admin1.jsp?pid="+cid+"&no="+no+"';</script>");
	}
	else{
