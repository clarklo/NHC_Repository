...
if(request.getParameter("id")!=null){
	...
	rs = op.query("SELECT * FROM leaveword1 WHERE l_id="+id);
	rs.next();
...
<table width="95%" border="1" align="center" cellpadding="3" cellspacing="0" bordercolorlight="#ECEEE4" bordercolordark="#CCCABC">
  <tr>
    ...
    <td width="13%" align="center" background="images/tablebg.gif"><strong>預約類型</strong></td>
	...
	int pageCount = 0; 
	//獲得記錄總數
	rs = op.query("SELECT COUNT(*) AS total FROM leaveword1");
	while(rs.next()){
	...
	//rs = op.query("SELECT top " + pageSize + " * FROM cms_link where cms_link_id NOT IN (SELECT TOP " + ((currPage - 1) * pageSize)+ " cms_link_id from cms_link ORDER BY cms_link_id DESC) ORDER BY cms_link_id DESC");
 	//##mysql##rs = op.query("SELECT * FROM leaveword1 ORDER BY l_id DESC limit "+((currPage - 1) * pageSize)+"," + pageSize);
	rs = op.query("SELECT * FROM leaveword1 ORDER BY l_id DESC ," + pageSize);
  %>
