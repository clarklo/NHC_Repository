...
//	}
	...
	if(PayCont.PAY_003.value == ""){
		alert("自費民眾共同診療項目費用不能為空,請輸入自費民眾共同診療項目費用!");
		PayCont.PAY_003.focus();
		return false;
	}              
	if(PayCont.PAY_004.value == ""){
		alert("國際醫療費用_共同診療項目不能為空,請輸入國際醫療費用_共同診療項目!");
		...
		alert("自費民眾共同診療項目費用需為數字,請輸入數字!");
		PayCont.PAY_003.focus();
		return false;
	}        
        if(isNaN(PayCont.PAY_004.value)){
		alert("國際醫療費用_共同診療項目需為數字,請輸入數字!");
		PayCont.PAY_004.focus();
		return false;
	}        
...
function del_check(id,p,payType)
{
 ...
 function resetClick()
{
    self.location.href="PayContAdmin2.jsp" ;
}
function selected(){
	PayCont.PAY_TYPE_name.focus();
}
...
<table width="95%" border="1" cellspacing="0" cellpadding="3" align="center" bordercolorlight="#ECEEE4" bordercolordark="#CCCABC">
  ...
  <tr> 
    <td height="25" align="center"><div align="right">自費民眾共同診療項目費用</div></td>
    <td height="25" align="center"><div align="left">
      <input name="PAY_003" type="text" id="PAY_003" size="40" maxlength="3" value="<%=updPAY_003%>">
    <span class="style1">*</span></div></td>
  </tr>
  <tr> 
    <td height="25" align="center"><div align="right">國際醫療費用_共同診療項目</div></td>
    <td height="25" align="center"><div align="left">
  ...
  <tr> 
    <td height="25" colspan="2" align="center"><div align="center">
       <input name="PAY_005" type="hidden" id="PAY_005" value="0" >
        <input name="PAY_006" type="hidden" id="PAY_006" value="0" >
        <input name="PAY_007" type="hidden" id="PAY_007" value="0" >
      <input name="PAY_008" type="hidden" id="PAY_008" value="0" >
      <input name="PAY_CONT_TYPE" type="hidden" id="PAY_CONT_TYPE" value="B" >
...
<table width="95%" border="1" align="center" cellpadding="3" cellspacing="0" bordercolorlight="#ECEEE4" bordercolordark="#CCCABC">
  <tr>
    ...
    <td width="4%" align="center" background="images/tablebg.gif"><strong>自費民眾共同診療項目費用</strong></td>
    <td width="4%" align="center" background="images/tablebg.gif"><strong>國際醫療費用_共同診療項目</strong></td>
	...
	int pageCount = 0; 
	//獲得記錄總數
	rs = op.query("SELECT COUNT(*) AS total FROM PAYCONT where PAY_CONT_TYPE='B' ");
	while(rs.next()){
  ...
  <%
  rs1 = op.query("SELECT row_number() over(order by pay_id) as rn,PAYCONT.* FROM PAYCONT  where PAY_CONT_TYPE='B'  order by PAY_id ");
  int j = 0;
  while(rs1.next()){
  ...
  <tr>
    ...
    <td align="center" height="25"><a href="PayContAdmin2.jsp?UpdateID=<%=PAY_id%>">修 改| <a onClick="javascript:del_check(<%=PAY_id%>,'<%=PAY_TYPE_CHT%>','B')" class="A">刪 除</a></a>  </td>
  </tr>
  <%
    }
  }
